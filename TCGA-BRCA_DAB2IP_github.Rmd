...
```{r,echo=F}
pdf("DAB2IP_expression_by_BRCA_subtype_boxplots_010924.pdf")
combined %>%
	select(SampleID,Expression,SUBTYPE) %>%
	group_by(SUBTYPE) %>%
	ggplot(aes(x=SUBTYPE,y=Expression,group=SUBTYPE,fill=SUBTYPE)) +
...
```{r,echo=F}
pdf("DAB2IP_expression_with_RORP_by_BRCA_subtype_boxplots_010924.pdf")
inner_join(TCGAexpr,inner_join(RORP,combined,by="SampleID"),by="SampleID") %>%
 	dplyr::select(SampleID,Gene,Expression.x,DAB2IP,Expression.y,ERstatus,SUBTYPE,RORPavg) %>%
 	dplyr::rename("Expression" = Expression.x,"DAB2IP_expression" = Expression.y) %>%
	dplyr::filter(Gene=="DAB2IP") %>%
...
```{r,echo=F}
pdf("DAB2IP_high_low_DEGs_expression_SUBTYPES_heatmap_LumADE_ordered_SubtypeThenDAB2IPexp_010924.pdf",width=20)
inner_join(TCGAexpr_DEG,combined,by="SampleID") %>%
 	dplyr::select(SampleID,Gene,Expression.x,DAB2IP,Expression.y,ERstatus,SUBTYPE) %>%
 	dplyr::rename("Expression" = Expression.x,"DAB2IP_expression" = Expression.y) %>%
 	dplyr::filter(DAB2IP=="DAB2IP_high" | DAB2IP=="DAB2IP_low") %>%
...
```{r,echo=F}
pdf("TCGA_BRCA_DAB2IP_highVslow_DEG_corHeatmap_splitBySubype_LumAlowVSall_010924.pdf",height=4,width=13)
ComplexHeatmap::Heatmap(cors[colnames(cors)[row_order],],
	show_row_names = F,
	show_column_names = F,
	cluster_rows = F, 
...
```{r,echo=F}
pdf("DAB2IP_status_vs_ProlifPscore_bySubtype_010924.pdf",width=12,height=6)
left_join(combined,PDscores) %>%
	dplyr::select(SampleID,ERstatus,SUBTYPE,D_Score,P_score_11genesMean,Expression,DAB2IP) %>%
	dplyr::filter(ERstatus=="Positive") %>%
	drop_na(SUBTYPE) %>%
...
```{r,echo=F}
pdf("DAB2IP_high_low_DEGs_expression_SUBTYPES_heatmap_LumADE_ordered_SubtypeThenDAB2IPexp_042922_NFkBtargets_010924.pdf",width=20)
inner_join(TCGAexpr_DEG,combined,by="SampleID") %>%
 	dplyr::select(SampleID,Gene,Expression.x,DAB2IP,Expression.y,ERstatus,SUBTYPE) %>%
 	dplyr::rename("Expression" = Expression.x,"DAB2IP_expression" = Expression.y) %>%
 	dplyr::filter(DAB2IP=="DAB2IP_high" | DAB2IP=="DAB2IP_low") %>%
...
```{r,echo=F}
pdf("Angana_TCGA_BRCA_ERpositive_DAB2IPhigh_vs_DAB2IPlow_LumA_DESeq2_UP_overlap_RELA_5_breast_gt10_boxplots_bySubtype_v5_010923.pdf",width=10,height=10)
inner_join(TCGAexpr_DEG,combined,by="SampleID") %>%
	dplyr::filter(Gene %in% goi) %>%
...
```{r,echo=F}
pdf("DAB2IP_high_low_DEGs_expression_SUBTYPES_heatmap_LumADE_ordered_SubtypeThenDAB2IPexp_DN_KEGGspliceosome_010924.pdf",width=20,height=6)
inner_join(TCGAexpr_DEG,combined,by="SampleID") %>%
 	dplyr::select(SampleID,Gene,Expression.x,DAB2IP,Expression.y,ERstatus,SUBTYPE) %>%
 	dplyr::filter(Gene %in% kegg.splice.genes) %>%
 	dplyr::rename("Expression" = Expression.x,"DAB2IP_expression" = Expression.y) %>%
